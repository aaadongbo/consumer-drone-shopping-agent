...
Use `AI_REVIEW_NEEDS_CHANGES` for any actionable correctness, scope, test, evidence, risk, or record defect; list severity and file/line evidence. Use `BLOCKED` only when evidence is unavailable or an authority boundary prevents a conclusion. Use `AI_REVIEW_PASS` only when no actionable finding remains, and include canonical identity, exact range, mode, digest, reviewed risk tier, verification evidence, residual risk, and the next gate. The reviewer must not infer direct main integration or push authority from LOW auto-advance eligibility or from feature-delivery planning text.
